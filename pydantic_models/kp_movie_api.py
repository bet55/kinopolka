...
class KPFilmModel(BaseModel):
    ...
    budget: Optional[int] = Field(
        0, validation_alias=AliasChoices("", AliasPath("budget", "value"))
    )
    fees: Optional[int] = Field(
        0, validation_alias=AliasChoices("", AliasPath("fees", "world", "value"))
    )
    premiere: Optional[str] = Field(
        "1970-01-01T00:00:00.000Z", validation_alias=AliasPath("premiere", "world")
    )
    ...
    rating_imdb: Optional[float] = Field(
        0.0, validation_alias=AliasPath("rating", "imdb")
    )
