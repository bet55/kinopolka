    ...
    async def link_existing_poster(self, movie, kp_id):
        ...
        for ext in ['.jpg', '.png']:
            ...
            if poster_path.exists():
                ...
                await sync_to_async(movie.poster_local.save)(file_name, ContentFile(poster_path.read_bytes()), save=True)
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                if e.response.status_code in (404, 403):
                    self.stdout.write(self.style.WARNING(f"Poster not found for movie {kp_id}: {str(e)}"))
                    break  # No retry for 404 or 403
                elif e.response.status_code == 429:
                    self.stdout.write(self.style.WARNING(f"Rate limit hit for movie {kp_id}, attempt {attempt}/{max_retries}"))
                    if attempt < max_retries:
                        await asyncio.sleep(2 ** attempt)  # Exponential backoff
                    else:
                        self.stdout.write(self.style.ERROR(f"Max retries reached for movie {kp_id}: {str(e)}"))
                        break
                else:
                    self.stdout.write(self.style.ERROR(f"HTTP error for movie {kp_id}: {str(e)}"))
                    break
            except (httpx.RequestError, httpx.TimeoutException) as e:
                self.stdout.write(self.style.WARNING(f"Network error for movie {kp_id}, attempt {attempt}/{max_retries}: {str(e)}"))
                if attempt < max_retries:
                ...
                self.stdout.write(self.style.ERROR(f"Unexpected error for movie {kp_id}: {str(e)}"))
                break
